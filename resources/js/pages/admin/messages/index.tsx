...
export default function AdminMessages({ sessionsWithChats = [], users = [], messages = [] }: AdminMessagesProps) {
    ...
    const handleSessionSelect = (session: SessionWithChat) => {
        ...
        // If this is a new conversation, don't navigate yet - wait for user to send first message
        if (!session.id.toString().startsWith('new_')) {
            // Navigate to the conversation
            router.visit(`/admin/messages/${session.id}`);
        }
    };
    ...
    const handleSendMessage = () => {
        if ((newMessage.trim() || selectedFile) && selectedSession) {
            ...
            const sessionId = selectedSession.id.toString().startsWith('new_')
                ? selectedSession.id.toString().replace('new_', '')
                : selectedSession.id;

            router.post(`/admin/messages/${sessionId}`, formData, {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
                    setNewMessage('');
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">Communicate with all users in the system</p>
                    </div>
                </div>
                                    ...
                                    {filteredUsers.length > 0 ? (
                                        filteredUsers.map((user) => {
                                            ...
                                            const hasConversation = sessionsWithChats.some(session =>
                                                session.client_name === user.name || session.client_email === user.email
                                            );
                ...
                <div className="flex h-[calc(100vh-18rem)] md:h-[calc(100vh-16rem)] bg-card rounded-xl border overflow-hidden md:flex">
                    ...
                    {/* Desktop Sidebar */}
                    <div className="hidden md:flex w-1/3 bg-background border-r border-border flex-col min-w-0">
                            ...
                            <>
                                ...
                                <div className="p-4 bg-background border-b border-border flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <Link
                                            href={`/admin/messages/${selectedSession.id}`}
                                    ...
                                    <div className="max-w-4xl mx-auto space-y-4">
                                        {messages && messages.length > 0 ? (
                                            messages.map((message) => (
                                                <div key={message.id} className={`flex ${message.is_from_admin ? 'justify-end' : 'justify-start'}`}>
