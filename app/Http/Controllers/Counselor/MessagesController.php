...
class MessagesController extends \App\Http\Controllers\Controller
{
    public function index(Request $request)
    {
        ...
        // Get all sessions where this counselor has chats
        ...
        $sessionsWithChats = CounselingSession::with(['client', 'chats' => function($query) {
        ...
        ->map(function ($session) {
            ...
            $unreadCount = Chat::where('session_id', $session->id)
                ->where('sender_id', '!=', $session->counselor_id)
                ->where('is_read', false)
                ->count();
        ...
        return Inertia::render('counselor/messages/index', [
    ...
    public function show(Request $request, CounselingSession $session)
    {
        ...
        if ($session->counselor_id !== $request->user()->id) {
            abort(403);
        }
        ...
        // Check if this is the first time the counselor is accessing this session
        $hasStartedSession = Chat::where('session_id', $session->id)->exists();

        if (!$hasStartedSession) {
            // Send notification to client that counselor has started the session
            \App\Models\AppNotification::create([
                'user_id' => $session->client_id,
                'type' => 'session_started',
                'title' => 'Counselor Started Session',
                ...
                'data' => [
                    ...
                    'counselor_name' => $request->user()->name,
        ...
        return redirect()->route('counselor.messages.index', ['session' => $session->id]);
    }
